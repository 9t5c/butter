...
export class Magnetic {
  ...
  render = (cb: (val: boolean) => void) => {
    ...
    if (distanceFromMouseToCenter < this.triggerArea) {
      ...
      this.setMagneticState(true, cb)
    } else {
      this.setMagneticState(false, cb)
    }
  ...
  setMagneticState = (current: boolean, cb: (val: boolean) => void) => {
    if (current !== this.magnetState) {
      cb(current)
      this.magnetState = current
    }
  }
